...
def get_ai_summary(title, content, is_official=False, today_he=None, next_match_he=None):
    ...
    if is_official:
        ...
        humor_rule = (
            "😏 חריג להומור (משפט סיום בלבד, רק במקרים האלה):\n"
            "אם הכתבה עוסקת בשיפוט — זהות שופט המשחק הקרוב, החלטות שיפוט שנויות במחלוקת, ערעורים על עונשי משמעת — "
            "מותר ורצוי להוסיף משפט סיום קצר עם נימה צינית/הומוריסטית מהזווית של אוהד. "
            "דוגמאות לסגנון (אל תעתיק מילולית — צור משפט חדש): "
            "'נקווה שהפעם השריקה תזרום עם הכחולים', 'נראה אם הפעם תרגיש פחות מכור', "
            "'יישר לב — כבר שמענו את הסיפור הזה'. "
            "רק בכתבות שיפוט, רק משפט אחד, ורק בסוף — אל תפזר ציניות בכל התקציר.\n"
        )
        prompt = (
