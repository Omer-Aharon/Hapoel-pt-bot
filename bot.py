...
def is_article_main_topic_hapoel_pt(title, content):
    ...
    prompt = (
        ...
        "🎯 **הקריטריון המרכזי**: האם הפועל פתח תקווה היא הנושא המרכזי של "
        "האירוע/הסיפור שהכתבה מדווחת עליו?\n\n"
        "✅ ענה YES כש:\n"
        "- האירוע המרכזי בכתבה הוא משחק של הפועל פ\"ת (ניצחון, הפסד, סיכום, תחזית)\n"
        "- הסיפור הוא על שחקן/מאמן של הפועל פ\"ת (ראיון, החתמה, פציעה, פיטורים)\n"
        "- ניתוח של מצב הפועל פ\"ת בליגה, תכניות, החלטות הנהלה\n"
        "- אוהדים, אצטדיון מלאבס, אירועי מועדון\n\n"
        "❌ ענה NO כש:\n"
        "- האירוע המרכזי הוא של קבוצה אחרת, ורק מזכירים את הפועל פ\"ת בקונטקסט\n"
        "  (למשל: 'קבוצה X ניצחה את Y, ובמשחק הבא תפגוש את הפועל פ\"ת')\n"
        "- ראיון/דברי מאמן של קבוצה אחרת שמדבר על הפועל פ\"ת בחטף\n"
        "- סקירה כללית של הליגה / מחזור / טבלה שמזכירה את הפועל פ\"ת\n"
        "- כתבה על מכבי פתח תקווה (קבוצה אחרת לחלוטין!)\n\n"
        "🔑 **כלל אצבע**: אם תסיר/י את האזכורים של הפועל פ\"ת מהכתבה, "
        "האם הסיפור עדיין נשמע שלם? אם כן → NO. אם לא → YES.\n\n"
        f"כותרת: {title}\n\n"
