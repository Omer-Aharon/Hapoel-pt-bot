...
def is_article_main_topic_hapoel_pt(title, content):
    """בודק עם Gemini אם הפועל פ"ת היא הנושא המרכזי (לא רק נושא משני/אזכור)."""
    if not GEMINI_API_KEY:
        return True
    ...
    prompt = (
        "האם כדאי לשלוח את הכתבה הבאה לערוץ אוהדי הפועל פתח תקווה?\n\n"
        "🎯 **השאלה היחידה**: האם הפועל פתח תקווה היא הקבוצה שעליה הכתבה מדווחת — "
        "הגיבורה/הסובייקט המרכזי של הסיפור?\n\n"
        "✅ ענה YES כש:\n"
        "- הכתבה מסכמת/חוזה/מנתחת משחק של הפועל פ\"ת (מהזווית שלנו)\n"
        "- הסיפור הוא על שחקן/מאמן של הפועל פ\"ת (ראיון, החתמה, פציעה, פיטורים)\n"
        "- ניתוח של מצב הפועל פ\"ת בליגה, תכניות, החלטות הנהלה\n"
        "- אוהדים, אצטדיון מלאבס, אירועי מועדון של הפועל פ\"ת\n\n"
        "❌ ענה NO גם כש הפועל פ\"ת **מוזכרת** בכתבה, אם הכתבה היא על קבוצה אחרת:\n"
        "- 'קבוצה X ניצחה את Y, ובמחזור הבא תפגוש את הפועל פ\"ת' — זו כתבה על X\n"
        "- ראיון/דברי מאמן/שחקנים של קבוצה אחרת על המשחק נגדנו או על הפועל פ\"ת — "
        "  סובייקט הסיפור הוא הקבוצה האחרת, לא אנחנו\n"
        "- מצב/דרמה/פציעות/מהומות בקבוצה אחרת לקראת המשחק נגדנו — זה סיפור על הקבוצה האחרת\n"
        "- 'ניצחון על הפועל פ\"ת יבטיח להם X' / 'X זקוקה לנקודות מולנו' — זה סיפור על X\n"
        "- סקירה כללית של ליגת העל / מחזור / טבלה שכוללת אותנו\n"
        "- כתבה על מכבי פתח תקווה (קבוצה אחרת לחלוטין!)\n\n"
        "🔑 **מבחן ה-'מי הגיבור'**: מי הקבוצה שכל הפעולות, ההחלטות והרגשות בכתבה "
        "מתייחסות אליה? אם זו לא הפועל פ\"ת — NO, גם אם שמנו מופיע 5 פעמים.\n\n"
        "🔑 **מבחן ההסרה**: אם תסיר/י את כל האזכורים של הפועל פ\"ת מהכתבה, האם נשאר "
        "סיפור שלם על קבוצה אחרת? אם כן → NO.\n\n"
        f"כותרת: {title}\n\n"
    ...
    if not answer:
        # Gemini לא זמין בכלל (גם fallback) — עדיף לפספס מאשר לשלוח לא מסונן.
        # ההיגיון: כתבות אמיתיות יחזרו בריצה הבאה (לא מסומנות seen), כי בשלב הזה
        # סינון הדומיין+הרלוונטיות הבסיסי כבר תפס את הכותרת — מה שעובר הלאה אבל
        # נופל פה הוא בדרך כלל כתבה שלא צריך לשלוח.
        print("DEBUG:   ⚠️ topic-check נכשל לחלוטין — לא שולחים (fail-closed)", flush=True)
        return False
...
def get_ai_summary(title, content, is_official=False, today_he=None, next_match_he=None):
    ...
    anti_fabrication_rule = (
        ...
        "• תוצאת משחק (ניצחון/הפסד/תיקו): העתק בדיוק את שיוך התוצאה לכל קבוצה כפי שכתוב "
        "בכתבה - אסור להפוך ניצחון להפסד או להפך, ואסור לשנות את סדר הקבוצות בציון. "
        "לפני שכותבים משפט שמציין מי ניצח - קרא שוב את הכתבה ווודא שהמסקנה (מי ניצח, "
        "ומה הניקוד של כל צד) תואמת בדיוק למה שכתוב בה, לא רק שהספרות עצמן מוכרות.\n"
        "• בספק - השמט. עדיף תקציר קצר ומדויק על פני ארוך וכוזב. "
        "כל פרט שלא בטוח לגביו = לא נכתב.\n\n"
    )

    voice_rule = (
        "📝 גוף הכתיבה: סיקור עיתונאי בגוף שלישי. "
        "אל תכתוב 'מועדוננו', 'אצלנו', 'מאמננו', 'משחקנו', 'אוהדינו', 'הכחולים שלנו' או כל ניסוח שמדבר מטעם הקבוצה. "
        "הקבוצה היא 'הפועל פתח תקווה' או 'הכחולים' — לא 'אנחנו'. "
        "אל תפתח במשפטי מסגרת כמו 'אוהדי הפועל מתעדכנים', 'בערוץ שלנו', 'חדשות לאוהדים' — היכנס ישר לתוכן הידיעה כאילו מדובר בכתבה בעיתון.\n"
    )

    tone_rule = (
        "🎙️ טון: עיתונאי חם — מקצועי, מדויק, זורם. לא יבש אבל גם לא רשמי-נוקשה. "
        "כותב כמו עיתונאי ספורט שמכיר את הקבוצה ואוהד אותה, אבל לא נציג רשמי שלה. "
        "הימנע מסלנג שכונתי, ביטויי הגזמה ('מטורף!', 'אדיר!', 'נורא!') וקלישאות. "
        "אפשר כינויי חיבה כמו 'הכחולים' או 'חבורתו של עומר פרץ' במידה.\n"
    )

    if is_official:
        prompt = (
            time_ctx + anti_fabrication_rule + voice_rule + tone_rule +
            "סכם את הודעת המועדון הבאה ב-4-5 משפטים. "
        ...
        humor_rule = (
            "😏 ציניות/עקיצה קלה — מותרת **כשמתבקש**, אסור להגזים:\n"
            "מתבקש = יש בכתבה משהו שמזמין הומור: שופט שכבר העניש אותנו בעבר, ראיון יומרני של שחקן/מאמן, "
            "החמצה בולטת, החלטת רוטציה תמוהה, שערוריית משמעת, או 'הבטחות גדולות' שלא ברור אם יתגשמו. "
            "כשמתבקש — מותר להוסיף משפט סיום אחד קצר עם עקיצה עדינה מנקודת מבט של אוהד. "
            "דוגמאות לסגנון (אל תעתיק מילולית — צור משפט חדש): "
            "'נקווה שהפעם השריקה תזרום עם הכחולים', 'נראה אם הפעם תרגיש פחות מכור', "
            "'נראה אם הוא יגבה את ההצהרה גם על הדשא', 'הבטחות יש — נחכה למחר ב-20:30 לראות מה גם יתממש'.\n"
            "🚫 כללי זהב:\n"
            "• ציניות תמיד אך ורק במשפט הסיום, אף פעם לא באמצע התקציר.\n"
            "• אסור על כל תקציר — רובם צריכים להישאר ענייניים. בלי טריגר ברור → אין עקיצה.\n"
            "• אסור על כתבות עצובות (פציעות חמורות, פטירות, מחלות, סיפורים אישיים) — שם מקסימום אמפתיה.\n"
            "• אסור אישי-פוגעני — לא להעליב מראה/אישיות/משפחה. רק עקיצה ספורטיבית.\n"
            "• מתון ולא בוטה — קצת אירוניה, לא בוז.\n"
        )
        prompt = (
...
def load_json(path, default):
    """טעינה גנרית של קובץ JSON, עם ברירת מחדל אם חסר/פגום"""
    ...
    try:
        ...
        print(f"DEBUG: {path} corrupted: {e}", flush=True)
        return default


def save_json(path, data):
    """שמירה גנרית של קובץ JSON"""
...
def describe_odds_movement(current, opening):
    """
    משווה יחסי הימורים נוכחיים מול קו הפתיחה (שנשמר בשעת הודעת ה-MatchDay).
    מחזיר בלוק טקסט לתנודות משמעותיות (≥0.05) או מחרוזת ריקה אם אין נתון פתיחה/שינוי זניח.
    """
    if not opening:
        return ""
    lines = []
    for key, label, is_draw in [
        ("hapoel_odds", "הפועל פ\"ת", False),
        ("draw_odds", "תיקו", True),
        ("opponent_odds", "היריבה", False),
    ]:
        try:
            curr, open_val = float(current[key]), float(opening[key])
        except (KeyError, TypeError, ValueError):
            continue
        diff = curr - open_val
        if abs(diff) < 0.05:
            continue
        trend = "" if is_draw else (" (התחזקו 📈)" if diff < 0 else " (נחלשו 📉)")
        lines.append(f"{label}: {open_val} ← {curr}{trend}")
    if not lines:
        return ""
    return "\n\n*תנודה מאז הבוקר:*\n" + "\n".join(lines)
...
def main():
    ...
    if match_today:
        ...
        if now_il.hour >= MATCHDAY_MIN_HOUR and f"matchday_{today_str}" not in tasks:
            ...
            # 🆕 שומרים קו פתיחה של ווינר (בבוקר) כדי שנוכל להשוות תנודה מול הסקר בהמשך היום
            if match_today.get("is_home") is not None:
                opening_odds = get_winner_odds(today_str, match_today["is_home"])
                if opening_odds:
                    save_json("odds_opening.json", {"date": today_str, **opening_odds})
                    print(f"DEBUG: 📊 נשמר קו פתיחה של ווינר: {opening_odds}", flush=True)
        ...
        if f"betting_{today_str}" not in tasks:
            ...
            if send_betting:
                if match_today.get("is_home") is not None:
                    ...
                    if odds:
                        opening = load_json("odds_opening.json", {})
                        movement_txt = describe_odds_movement(odds, opening) if opening.get("date") == today_str else ""
                        odds_text = (
